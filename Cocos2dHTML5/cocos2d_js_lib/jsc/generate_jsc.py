...
class CCPluginJSCompile():
    ...
    def init(self, options, workingdir):
        ...
        - `options`:
        """
        f = codecs.open("complier_config.json", "r", "utf-8")
        self._config = json.load(f)
        f.close()

        self._current_src_dir = None
        self._src_dir_arr = self.normalize_path_in_list(self._config["src_folders"])
        self._dst_dir = self._config["dst_folder"]
    ...
    def compile_js(self, jsfile, output_file):
        ...
        ret = subprocess.call(options['bin_exe_path'] + " " + jsfile+" "+output_file, shell=True)
        if ret == 0:
